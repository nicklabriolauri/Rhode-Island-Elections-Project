    ...
    candidatesEl.innerHTML=active.length?active.map(c=>{
      ...
      const ratingHtml=ratings.length?`<div class="candidate-detail"><div class="detail-label">Outside ratings & scorecards</div>${ratings.map(r=>{
        if(r.source_type==='state legislative effectiveness score'){
          const bills=r.bills||{};
          return `<section class="effectiveness-card"><div class="effectiveness-head"><div><span class="effectiveness-kicker">Legislative effectiveness · ${esc(r.year||'')}</span><strong>${esc(r.organization)}</strong></div><div class="effectiveness-score"><b>${esc(r.rating)}</b><span>Chamber average: ${esc(r.chamber_average??1)}</span></div></div><div class="effectiveness-context"><span>${esc(r.expectation||'')}</span><span>#${esc(r.party_rank)} among ${esc(r.comparison_group||'peers')}</span></div><div class="bill-funnel"><div><b>${esc(bills.introduced??'—')}</b><span>Introduced</span></div><div><b>${esc(bills.committee_action??'—')}</b><span>Committee action</span></div><div><b>${esc(bills.beyond_committee??'—')}</b><span>Beyond committee</span></div><div><b>${esc(bills.passed_chamber??'—')}</b><span>Passed chamber</span></div><div><b>${esc(bills.became_law??'—')}</b><span>Became law</span></div></div><p>${esc(r.note||r.measure||'')}</p><div class="effectiveness-links"><a href="${esc(r.source_url)}" target="_blank" rel="noopener">View source report ↗</a>${r.methodology_url?`<a href="${esc(r.methodology_url)}" target="_blank" rel="noopener">Methodology ↗</a>`:''}</div></section>`;
        }
