...
def apply_recount_aware_statuses(status):
    ...
    for r in results.get('races',[]):
        ...
        cs=sorted(cs,key=lambda c:Number(c.get('votes') or 0),reverse=True); total=sum(Number(c.get('votes') or 0) for c in cs); diff=Number(cs[0].get('votes') or 0)-Number(cs[1].get('votes') or 0)
        # R.I. Gen. Laws §17-19-37.1: under 20,000 votes, recount eligibility is less than the lesser of 2% of votes cast or 200 votes.
