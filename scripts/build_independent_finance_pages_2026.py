...
SUPPLEMENT={
 'Brittany M Kubicek': {'through':'June 30, 2026','receipts':16771.76,'spending':12753.07,'cash':4018.69,'liabilities':0.0,'source':'Rhode Island Board of Elections CF-2 / CF-4 filings (Q1 + Q2 2026)'},
...
EXPENDITURES={
 'Brittany M Kubicek':[
  ...
  ('2026-06-18','Advertising','Dasah Studios','Graphic design work',175.00),
  ('2026-05-17','Advertising','Dasah Studios','Graphic design work',175.00),
  ('2026-05-01','Advertising','Regine Printing Co., Inc','Palm cards',823.90),
  ('2026-06-17','Advertising','Regine Printing Co., Inc','Palmcard order',428.00),
  ('2026-04-02','Advertising','Regine Printing Co., Inc','Palm card order',411.95),
  ('2026-05-19','Advertising','Staples','Mother T letter printing and mailing',3392.38),
  ('2026-06-29','Fundraising Expenses',"Lang's Bowlarama",'Bowling fundraiser',230.00),
  ('2026-06-29','Fundraising Expenses',"Lang's Bowlarama",'Bowling fundraiser',153.99),
  ('2026-06-06','Fundraising Expenses',"Lang's Bowlarama",'Bowling fundraiser',470.00),
  ('2026-04-18','Fundraising Expenses','Eventeny','Pride tabling',245.08),
  ('2026-05-19','Fundraising Expenses','Eventeny','Providence Pride Parade',175.67),
  ('2026-04-28','Other','RI Internal Service Fund','Forfeiture of contribution received from out-of-state campaign committee',591.76),
  ('2026-03-26','Fundraising Expenses',"Chelo's",'Campaign launch fundraising party payment',1447.97),
  ('2026-03-14','Consultant & Professional Services','Sandy Dahari','Launch video',500.00),
  ('2026-03-11','Advertising','Regine Printing Co., Inc','Palm card order',315.65),
 ],
 'Jon D Brien':[
  ('2026-04-13','Donations (All Others)','Friends of Bob Phillips','Reimbursement connected to a Boy Scouts of America Troop 222 contribution',400.00),
  ('2026-04-13','Office Equipment & Supplies','Citizens Bank','Deposit slips',15.25),
  ('2026-03-12','Food, Beverages and Meals','Jon D Brien','Reimbursement for food, beverages and meals at a fundraiser at The District',640.00),
  ('2026-02-03','Fundraising Expenses','Jon D Brien','Reimbursement for Formatt Printing mailers and envelopes',141.24),
 ],
 'Jean P Barros':[
  ('2026-03-04','Donations (Political)','James Diossa','Political donation',250.00),
  ('2026-03-30','Donations (Political)','Donald Grebien','Political donation',150.00),
 ],
}
def apply_recount_aware_statuses(status):
    ...
    for r in results.get('races',[]):
        ...
        races[key]={'winner':norm(cs[0].get('name')),'recount_range':diff < threshold}
...
def spending_html(name):
    rows=EXPENDITURES.get(name,[])
    if not rows: return ''
    category={}
    for _,cat,_,_,amt in rows: category[cat]=category.get(cat,0)+amt
    cats=''.join(f'<div class="spend-category"><span>{html.escape(cat)}</span><b>{money(amt)}</b></div>' for cat,amt in sorted(category.items(),key=lambda x:x[1],reverse=True))
    tx=''.join(f'<tr><td>{date}</td><td>{html.escape(cat)}</td><td><strong>{html.escape(vendor)}</strong><small>{html.escape(purpose)}</small></td><td>{money(amt)}</td></tr>' for date,cat,vendor,purpose,amt in rows)
    return f'''<section class="panel detail"><div class="section-label">SPENDING DETAIL</div><h2>Where the money went</h2><p class="intro">Selected itemized expenditures from the 2026 filings currently held by RIEP. Category totals below cover the transactions displayed here; the official filing remains the authoritative complete record.</p><div class="spend-categories">{cats}</div><div class="table-wrap"><table><thead><tr><th>Date</th><th>Category</th><th>Payee / purpose</th><th>Amount</th></tr></thead><tbody>{tx}</tbody></table></div></section>'''

def page(c,fin):
    rawname=c['name']; name=html.escape(rawname); chamber='House' if c['chamber']=='house' else 'Senate'; dist=c['district']; office='State Representative' if c['chamber']=='house' else 'State Senator'
    if fin:
